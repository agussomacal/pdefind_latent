...
class Domain:
    ...
    def __init__(self, lower_limits_dict={}, upper_limits_dict={}, step_width_dict={}):
        if not (set(lower_limits_dict.keys()) == set(upper_limits_dict.keys()) == set(step_width_dict.keys())):
            kint = set(lower_limits_dict.keys()).intersection(set(upper_limits_dict.keys())).intersection(set(step_width_dict.keys()))
    ...
    def get_subdomain_from_index(self, axis_names_ixes):
        ...
        for ax_name, limits in axis_names_ixes.items():
            sub_domain.add_axis({ax_name: max(self.get_value_from_index(ax_name, limits[0]), self.lower_limits[ax_name])},  # to avoid going outside range.
    ...
    def get_subdomain(self, axis_names=None):
        ...
        """
        if axis_names is None:
            axis_names = {ax_name: [self.lower_limits[ax_name], self.upper_limits[ax_name]] for ax_name in self.axis_names}
        elif isinstance(axis_names, str):
            ...
            if all([isinstance(var, (Field, Variable, int, float)) for var in new_vars]):
                for var in new_vars:
                    if isinstance(var, (int, float)):
                        var = Variable(np.array(var), Domain(), domain2axis={}, variable_name=str(var))  # new constant variable
                    self.append(var)
