...
def merge_dicts(dict1, dict2, merge_func):
    ...
    return {k: merge_func(i for i in (dict1.get(k), dict2.get(k)) if i is not None) for k in set(dict1.keys()).union(set(dict2.keys()))}
...
def get_sorted_derivative_atoms(sym_expression):
    """
    return the derivatives in the expression sorted from lower derivatives to greater ones.
    :param sym_expression:
    :return:
    """
    der_atoms, func_atoms = get_atoms(sym_expression)
    max_derivative = max(der_atoms, key=lambda x: len(x.variables))

    derivatives_list = [max_derivative]
    for i in range(len(max_derivative.variables), 1, -1):
        derivatives_list.append(derivatives_list[-1].integrate())
    return list(reversed(derivatives_list))

def get_sorted_derivative_atoms2(sym_expression):
    """
    return the derivatives in the expression sorted from lower derivatives to greater ones.
    :param sym_expression:
    :return:
    ...
    if len(der_atoms) > 0:
        ...
        derivatives_list = der_atoms
    return func_atoms + list(reversed(derivatives_list))

#
# def get_func_for_ode(sym_x_expression, sym_y_expression):
#     symbols = sym_x_expression.free_symbols
#     assert len(symbols) == 1, "Only available for 1 dimensional domains: symbols={}".format(symbols)
#     # f2ix_dict = OrderedDict({})
#     # f2derivative_dict = OrderedDict({})
#     derivatives_list = get_sorted_derivative_atoms(sym_x_expression-sym_y_expression)
#
#     f2xindex_dict = OrderedDict({})
#     for i, f in enumerate(derivatives_list):
#         f2xindex_dict[f.integrate()] = i
#
#     # for i, f in enumerate(get_atoms(sym_x_expression)):
#     #     f2ix_dict[f] = i
#     #     f2derivative_dict[f] = f.diff()
#
#     def func(x, t):
#         x = np.array(x).ravel()
#         # assert len(x) == len(derivatives_list)
#         res = [np.nan] * len(x)
...
#
#         # for f, der_f in f2derivative_dict.items():
#         #     if der_f in f2ix_dict.keys():
#         #         res[f2ix_dict[f]] = x[f2ix_dict[der_f]]
#         # assert np.isnan(res).sum() == 1, "The equation needs more derivatives."
#
#         ix = np.where(np.isnan(res))[0][0]
#         res[ix] = sym_x_expression
#         for f, i in reversed(f2xindex_dict.items()):
...
def get_func_for_ode(sym_x_expression_list, sym_y_expression_list, regressors):
    ...
    #
    # def func(x, t):
    #     assert len(x) == len(f2derivative_dict)
    #     res = [np.nan] * len(x)
    #     for f, der_f in f2derivative_dict.items():
    #         if der_f in f2ix_dict.keys():
    #             res[f2ix_dict[f]] = x[f2ix_dict[der_f]]
    ...
    #     ix = np.where(np.isnan(res))[0][0]
    #     res[ix] = sym_x_expression
    #     for f, i in f2ix_dict.items():
    #         res[ix] = res[ix].subs({f: x[i]})
    #     res[ix] = res[ix].subs({symbol: t_val for t_val, symbol in zip(t, symbols)})  # TODO: assumes that domain values in t are in the same order that the free symbols
    #     res[ix] = float(res[ix])
    #     return res
